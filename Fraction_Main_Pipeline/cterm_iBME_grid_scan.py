...
path_structures = "/users/t/j/tjaglal/cterm_structures"
...
def concat_fractions(save_path):
    ...
    for i in range(num_gps):
        ...
        if missing_named_files:
            raise FileNotFoundError(f"Missing PDB-labelled calculation file: {missing_named_files[0]}")
        ...
        if len(final_df) != len(final_named_df):
            raise ValueError(f"GP{i} numeric and PDB-labelled calculation files have different row counts.")
        ...
        final_named_df.to_csv(
            os.path.join(compiled_dir, f"GP{i}_all_saxs_pdb.txt"), sep=' ', index=False, header=False
        )
        pd.Series(final_named_df.iloc[:, 0]).to_csv(
            os.path.join(compiled_dir, f"GP{i}_manifest.txt"), header=None, index=False
        )
...
run = subprocess.run(["{}/cterm_do_gp_fraction.sh".format(path_main), args.frame_path, "{}/SASDLS4_trun.dat".format(path_exp_file),
